...
if (typeof window.DestinationsManager !== "undefined") {
  ...
  class DestinationsManager {
    ...
    // ============================================
    // LOAD DESTINATIONS - WITH COMPLETE DATA INCLUDING RATES
    // ============================================
    async loadDestinations() {
      try {
        ...
        // FIRST, check if optional_tour_rates table has data
        const { data: testRates, error: testError } =
          await window.sns_supabase_client
            .from("optional_tour_rates")
            .select("*");

        console.log("🔍 TEST: All optional_tour_rates:", testRates);

        if (testError) {
          console.error("❌ Error fetching optional_tour_rates:", testError);
        }

        // MAIN QUERY - with proper nested selects
        ...
        this.destinations = (destinations || []).map((dest) => ({
          ...dest,
          destination_packages: (dest.destination_packages || []).map((pkg) => {
            console.log(`📦 Processing package: ${pkg.package_name}`);

            return {
              ...pkg,
              package_optional_tours: (pkg.package_optional_tours || [])
                .filter((item) => item.optional_tours)
                .map((item) => {
                  const tour = item.optional_tours;
                  const rates = tour?.optional_tour_rates || [];

                  console.log(`🎯 Tour found: ${tour?.tour_name}`, {
                    tour: tour,
                  ...
                  return {
                    ...
                    optional_tours: {
                      ...tour,
                      // Make sure rates is properly accessible
                      optional_tour_rates: rates,
                    },
                  };
                }),
            };
    ...
    async showDestinationModal(destinationId) {
      ...
      try {
        // Find destination in already loaded data
    ...
    renderCompleteModal(destination, packages) {
        ...
        <div class="bg-white p-5 rounded-xl border-2 border-gray-200 mb-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ${destination.destination_images
              .map(
                (img) => `
              <div class="relative group rounded-lg overflow-hidden aspect-square cursor-pointer" 
                   onclick="destinationsManager.openImagePreview('${img.url}')">
                <img src="${img.url}" alt="${img.alt_text || destination.name}" 
      ...
      // Re-attach book now button event listeners
      setTimeout(() => {
    ...
    // ============================================
    // RENDER OPTIONAL TOURS - FINAL FIXED VERSION
    // ============================================
    renderOptionalTours(pkg) {
      ...
      // Filter out any null entries and get valid tours
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ${validTours
              .map((item) => {
                ...
                // IMPORTANT: Get rates from optional_tour_rates
                const rates = tour.optional_tour_rates || [];
                const firstRate = rates.length > 0 ? rates[0] : null;

                console.log(`🎯 Tour: ${tour.tour_name}`, {
                  rates: rates,
                  firstRate: firstRate,
                });
                ...
                if (firstRate) {
                  ...
                  if (
                    firstRate.rate_6pax &&
                    parseFloat(firstRate.rate_6pax) > 0
                  )
                    rateItems.push(
                      `6 Pax: ₱${Number(firstRate.rate_6pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_7pax &&
                    parseFloat(firstRate.rate_7pax) > 0
                  )
                    rateItems.push(
                      `7 Pax: ₱${Number(firstRate.rate_7pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_8pax &&
                    parseFloat(firstRate.rate_8pax) > 0
                  )
                    rateItems.push(
                      `8 Pax: ₱${Number(firstRate.rate_8pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_9pax &&
                    parseFloat(firstRate.rate_9pax) > 0
                  )
                    rateItems.push(
                      `9 Pax: ₱${Number(firstRate.rate_9pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_10pax &&
                    parseFloat(firstRate.rate_10pax) > 0
                  )
                    rateItems.push(
                      `10 Pax: ₱${Number(firstRate.rate_10pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_11pax &&
                    parseFloat(firstRate.rate_11pax) > 0
                  )
                    rateItems.push(
                      `11 Pax: ₱${Number(firstRate.rate_11pax).toLocaleString()}`,
                    );
                  if (
                    firstRate.rate_12pax &&
                    parseFloat(firstRate.rate_12pax) > 0
                  )
                    rateItems.push(
                      `12 Pax: ₱${Number(firstRate.rate_12pax).toLocaleString()}`,
                    );
                  if (
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:shadow-md transition">
                  ...
                  <button onclick='destinationsManager.showTourDetails(${JSON.stringify(tour).replace(/'/g, "\\'")}, ${JSON.stringify(firstRate || {}).replace(/'/g, "\\'")})' 
                          class="w-full mt-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white py-2 rounded-lg text-sm font-medium hover:shadow-md transition">
    ...
    // ============================================
    // OPEN BOOKING MODAL - WITH FIXED TOUR RATES
    // ============================================
    async openBookingModal(packageName, packageId) {
      ...
      // Process optional tours to ensure rates are accessible
      const optionalTours = (pkg.package_optional_tours || [])
        ...
        .map((item) => ({
          ...
          optional_tours: {
            ...item.optional_tours,
            // Use optional_tour_rates directly
            optional_tour_rates: item.optional_tours?.optional_tour_rates || [],
          },
        }));

      console.log(
        "🎯 Processed optional tours:",
        optionalTours.map((t) => ({
          name: t.optional_tours?.tour_name,
          rates: t.optional_tours?.optional_tour_rates,
        })),
      );
      ...
      // ============================================
      // OPTIONAL TOURS HTML - FIXED VERSION
      // ============================================
      ...
      if (optionalTours && optionalTours.length > 0) {
        ...
        optionalTours.forEach((item) => {
          ...
          console.log(`🎯 Booking modal - Tour: ${tour.tour_name}`, {
            rates: rates,
            firstRate: firstRate,
          });
    ...
    // ============================================
    // SHOW TOUR DETAILS - FIXED VERSION
    // ============================================
    showTourDetails(tour, rates) {
      ...
      // Build rate items - use the rates parameter
      const rateItems = [];
      const rateFields = [
                ...
                <div class="flex items-center gap-4 text-sm">
                  ${
                    ...
                    <span class="flex items-center gap-1">
                      <i class="far fa-clock"></i> ${tour.duration_hours} hours
                    </span>
                  `
                      : ""
                  }
    ...
    async refreshDestinations() {
      ...
      try {
        ...
        this.destinations = (destinations || []).map((dest) => ({
          ...dest,
          destination_packages: (dest.destination_packages || []).map(
            (pkg) => ({
              ...pkg,
              package_optional_tours: (pkg.package_optional_tours || [])
                .filter((item) => item.optional_tours)
...
console.log(
  "✅ Tours.js loaded - COMPLETE FIXED VERSION WITH OPTIONAL TOUR RATES!",
);
